...
do
	...
	function Funcs:AddDropdown(Idx, Info)
		...
		local Dropdown = {
			Values = Info.Values,
			OriginalValues = Info.Values, -- Store original values for combobox filtering
			Value = Info.Multi and {},
			Multi = Info.Multi,
			Type = "Dropdown",
			SpecialType = Info.SpecialType, -- can be either 'Player' or 'Team'
			Combobox = Info.Combobox or false,
			Callback = Info.Callback or function(Value) end,
		}
		...
		if Dropdown.Combobox then
			...
			ItemInput = Library:Create("TextBox", {
				BackgroundTransparency = 1,
				Position = UDim2.fromOffset(0, 0),
				Size = UDim2.fromScale(5, 1),
				Font = Library.Font,
			...
			Library:ApplyTextStroke(ItemInput)
			Library:AddToRegistry(ItemInput, {
				TextColor3 = "FontColor",
			})

			ItemInput:GetPropertyChangedSignal("Text"):Connect(function()
				local searchText = ItemInput.Text:lower()
				if searchText == "" then
					Dropdown.Values = Dropdown.OriginalValues
				else
					local filtered = {}
					for _, value in next, Dropdown.OriginalValues do
						if string.find(value:lower(), searchText, 1, true) then
			...
			ItemInput.Focused:Connect(function()
			...
			local function UpdateCursor()
				...
				if not ItemInput:IsFocused() or ItemInput.TextBounds.X <= reveal - 2 * PADDING then
					ItemInput.Position = UDim2.new(0, PADDING, 0, 0)
				else
					local cursor = ItemInput.CursorPosition
					if cursor ~= -1 then
						local subtext = string.sub(ItemInput.Text, 1, cursor - 1)
						...
						local currentCursorPos = ItemInput.Position.X.Offset + width

						if currentCursorPos < PADDING then
							ItemInput.Position = UDim2.fromOffset(PADDING - width, 0)
						elseif currentCursorPos > reveal - PADDING - 1 then
							ItemInput.Position = UDim2.fromOffset(reveal - width - PADDING - 1, 0)
						end
					end
				end
			end
			...
			ItemInput:GetPropertyChangedSignal("Text"):Connect(UpdateCursor)
			ItemInput:GetPropertyChangedSignal("CursorPosition"):Connect(UpdateCursor)
			ItemInput.FocusLost:Connect(UpdateCursor)
			ItemInput.Focused:Connect(UpdateCursor)
		else
			ItemList = Library:CreateLabel({
				Position = UDim2.new(0, 5, 0, 0),
				Size = UDim2.new(1, -5, 1, 0),
				TextSize = 14,
				Text = "--",
		...
		function Dropdown:Display()
			...
			if Dropdown.Combobox and ItemInput then
				if not ItemInput:IsFocused() then
					ItemInput.Text = Str
				end
		...
		function Dropdown:SetValues(NewValues)
			if NewValues then
				Dropdown.OriginalValues = NewValues
		...
		-- Only handle click on outer frame if not combobox (combobox uses TextBox focus)
		if not Dropdown.Combobox then
